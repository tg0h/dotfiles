...
def handle_result(args: List[str], answer: str, target_window_id: int, boss: Boss) -> None:
    ...
    # is_focussed=False
    # if boss.active_tab.title == 'candy':
    #     is_focussed=True
    ...
    if tab:
        colour_tab(boss, tab)
        is_candy_foccused=False
        if boss.active_tab.title == 'dotfiles':
            is_candy_foccused=True
        if not is_candy_foccused:
...
def get_wiki_tab(boss: Boss):
    tabs = boss.match_tabs('title:^dotfiles')
