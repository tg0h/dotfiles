        ...
        if opts ~= "auto" then
            for _, keymap in ipairs(keymaps) do
                vim.api.nvim_create_autocmd("BufEnter", {
                    ...
                    group = group,
                    callback = function()
                        vim.keymap.set(keymap[1], keymap[2], function() ls.snip_expand(snippet) end,
...
local if_snippet = s({trig = "IF", regTrig = false, hidden = true}, c(1, {if_fmt_1, if_fmt_2})) -- }}}
local function_fmt = fmt( -- {{{
[[
function {}({}) {{
  {}
}}
    ]], {i(1, "myFunc"), c(2, {i(1, "arg"), i(1, "")}), i(3, "//TODO:")})
...
{trig = "for([%w_]+)", regTrig = true, hidden = true}, fmt([[
for (let {} = 0; {} < {}; {}++) {{
  {}
}}
