...
require'nvim-tree'.setup {
    ...
    view = {
        ...
        mappings = {
            custom_only = true,
            list = {
                {key = {'<CR>', 'o', '<2-LeftMouse>'}, cb = tree_cb('edit')}, {key = '<C-e>', action = 'edit_in_place'},
                {key = 'O', action = 'edit_no_picker'}, {key = '<C-v>', cb = tree_cb('vsplit')},
                {key = '<C-x>', cb = tree_cb('split')}, {key = '<C-t>', cb = tree_cb('tabnew')},
                {key = '<Tab>', cb = tree_cb('preview')}, -- useful home row mappings
                {key = 'D', cb = tree_cb('parent_node')}, {key = 'h', cb = tree_cb('close_node')},
                {key = 'H', cb = tree_cb('collapse_all')}, {key = 't', cb = tree_cb('next_sibling')},
                {key = 'T', cb = tree_cb('last_sibling')}, {key = 'n', cb = tree_cb('prev_sibling')},
                {key = 'N', cb = tree_cb('first_sibling')}, {key = 'l', action = 'edit', action_cb = edit_or_open},
                {key = 'L', action = 'vsplit_preview', action_cb = vsplit_preview}, {key = '-', cb = tree_cb('dir_up')},

                {key = 'E', action = 'expand_all'}, {key = {'<2-RightMouse>', '<C-]>', '_'}, cb = tree_cb('cd')},

                {key = '<A-g>', cb = tree_cb('prev_git_item')}, {key = '<A-r>', cb = tree_cb('next_git_item')},

                {key = '[e', action = 'prev_diag_item'}, {key = ']e', action = 'next_diag_item'},

                {key = 'I', action = 'toggle_git_ignored'}, {key = 'i', cb = tree_cb('toggle_ignored')},
                {key = '.', cb = tree_cb('toggle_dotfiles')}, {key = 'U', action = 'toggle_custom'},

                {key = 'A', cb = tree_cb('search_node')}, {key = 'R', cb = tree_cb('refresh')},
                ...
                {key = 'gy', cb = tree_cb('copy_absolute_path')}, {key = '\'', cb = tree_cb('system_open')}, -- avoid clash with lightspeed

                {key = 'q', cb = tree_cb('close')}, {key = 'u', action = 'toggle_file_info'},
                {key = 'g?', cb = tree_cb('toggle_help')}, {key = 'f', action = 'live_filter'},
